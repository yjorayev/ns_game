...
import { JumpDescriptor } from '../common/classes/JumpDescriptor';
...
export class BoardService {
    ...
    public getPath(boardState: BoardState, from: Location, to: Location): Path {
        const queue = [{
            jump: { location: from, canChangeDirection: true },
        ...
        while (queue.length > 0) {
            ...
            if (lastItem.jump.location.row === to.row && lastItem.jump.location.column === to.column) {
